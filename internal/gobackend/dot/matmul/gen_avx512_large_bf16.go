...
// - Base source file (edit this one): avx512_large.go
...
func avx512LargeKernelBFloat16( //alt:bf16
	...
	//alt:f32 packedOutput []float32,
	packedOutput []float32, //alt:bf16
	...
	for lhsRowIdx := 0; lhsRowIdx < lhsActiveRows; lhsRowIdx += kernelRows {
		...
		for rhsColIdx := 0; rhsColIdx < rhsActiveCols; rhsColIdx += kernelCols {
			...
			_ = rhsRegisterStride // Not used by bf16 path, since we load only one 512-bit register at a time. //alt:bf16
			...
			for range contractingLen {
				...
				//alt:float32 rhsPtr1 := unsafe.Pointer(rhsRowPtr + rOffset + rhsRegisterStride)
				//alt:float32 rhsVec0 := archsimd.LoadFloat32x16((*[16]float32)(rhsPtr0))
				//alt:float32 rhsVec1 := archsimd.LoadFloat32x16((*[16]float32)(rhsPtr1))
				rhsBF16 := bfloat16.LoadBFloat16x32((*[32]bfloat16.BFloat16)(rhsPtr0)) //alt:bf16
				rhsVec0, rhsVec1 := rhsBF16.ToFloat32()                                //alt:bf16
				rOffset += rhsStride

				// Row 0
				//alt:float32 lhsVal0 := *((*float32)(unsafe.Pointer(lhsRowPtr + lOffset + 0)))
